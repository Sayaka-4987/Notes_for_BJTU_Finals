...
1. 提高相连性，降低两个存储块竞争一个Cache位置的概率

2. 向层次结构中再增加额外的层次，多级Cache

   - 初级Cache要最小化命中时间
     - 因此初级Cache通常采用较小的块和较小的cache大小
...
虚拟存储器即用主存作为二级存储的“cache”，通常用磁盘实现

使用虚拟存储器技术的动机：

1. 允许多个程序高效、安全地在多个程序之间共享内存
   - 主存仅载入很多程序的活跃部分
   - 程序之间必须互相保护，确保一个程序只能读写主存分配给它的区域
   - 虚拟内存要实现程序的地址空间和物理地址的转换
2. 允许单个用户程序超过主存储器的大小
   - 以前当程序过大时，程序员将程序手动分成片段，还得辨别出互相独立的片段
   - 虚拟存储器就能自动管理2级存储层次(主存和次级存储)

虚拟存储器的块称为页，虚拟存储器缺失称为页故障
...
虚拟内存通过提供重定位（relocation）来简化程序的加载执行：
...
快表TLB需要考虑是不是有index和tag以及物理地址划分
...
1. **块放在哪里** Where can a block be placed?
2. **如何找到一个块** How is a block found?
3. **缓存缺失时，哪个块应该被替换** Which block should be replaced on a cache miss?
4. **写入时发生了什么** What happens on a write?
...
虚存在技术上只能用写回（硬盘读写开销太大了），现代Cache也倾向于使用写回
...
1. **强制失效**：程序运行之初数据都在主存，不在cache中，所以第一次访问一定不能命中Cache
2. **容量失效**：Cache容量不足，程序需要的块不能全部放进，需要的块之前在Cache，后来被调出了Cache
   - <u>全相连</u> 才有容量失效的说法
