...
<img src="media/image-20220303114744885.webp" alt="image-20220303114744885" style="zoom: 33%;" />
...
<img src="media/Peterson算法.webp" style="zoom: 33%;"  >
...
- 面向用户的准则
  - 周转时间短
    ...
    - 平均带权周转时间 $W=\frac{1}{n}[\sum_{i=1}^n \frac{T_i}{T_{S_i}}]$ （$\frac{T_i}{T_{S_i}}$ 即 $周转时间/服务时间$）
  - 响应时间快
  - 截止时间的保证
  - 优先权准则
- 面向系统的准则
...
系统状态 S 为死锁状态的**充要条件**是 <u>当且仅当</u> 该状态下的资源分配图 RAG 是**不可完全化简**的。
...
<img src="media/image-20220622132317999.webp" alt="image-20220622132317999" style="zoom:33%;" />
...
| 技术     | 简要说明                                                                 | 优点                                     | 缺点                         |
|:------ | -------------------------------------------------------------------- | -------------------------------------- | -------------------------- |
| 固定分区   | 主存被分为很多大小固定的分区，进程可以装载到大于等于自身大小的分区。                                   | 实现简单                                   | 1.有内部碎片<br />2.活动进程的数目是固定的 |
| 动态分区   | 分区是被动态创建的，进程可以装载到正好等于自身大小的分区。                                        | 没有内部碎片<br />内存使用更完全                    | 有外部碎片，需要压缩外部碎片             |
| 简单分页   | 主存被分为很多大小相同的帧，进程被分为很多与帧大小相同的页。<br />要装入一个进程，需要将进程所有的页装入主存，可以是不连续的帧中。 | 没有外部碎片                                 | 有很少的内部碎片（仅出现在进程的最后一页）      |
| 简单分段   | 进程被分为很多的段，要装入一个进程，需要将进程所有的段装入主存中不一定连续的动态分区。                          | 没有内部碎片<br />比起动态分区，内存利用率更高，开销小         | 有外部碎片，需要压缩外部碎片             |
| 虚拟内存分页 | 与简单分页相比，不需要将进程的所有页装入主存                                               | 没有外部碎片<br />巨大的虚拟内存空间<br />更高程度的多道程序设计 | 复杂的内存管理开销                  |
...
<img src="media/固定分区分配方式.webp" style="zoom: 33%;" />
...
若假定快表查找命中率为80%，则其有效访问时间为 120×80%＋220 ×(1－80%) =140ns
...
按数据传输速率分类：高速设备、中速设备、低速设备

按信息交换单位分类：块设备（传输速率高、可寻址、DMA），字符设备（传输速率低、不可寻址、中断）

按设备共享属性分类：独占设备、共享设备、虚拟设备

<img src="media/image-20220520175356021.webp" alt="image-20220520175356021" style="zoom:50%;" />
...
引入通道的目的在于提高 I/O 过程的独立性，使数据的传送能独立于 CPU；

通道为特殊的处理机，其指令类型单一、无专有内存（与 CPU 共享内存）执行通道 I/O 程序控制 I/O 

#### 通道指令

- 读指令：外设 `=>` 内存
- 写指令：内存 `=>` 外设
- 控制指令：具体功能与所控设备有关，如磁带机反绕、磁盘机寻道、打印机换页等
...
<img src="media/image-20220520180254743.webp" alt="image-20220520180254743" style="zoom: 50%;" />

#### 中断驱动 I/O 控制方式

引入中断机制

<img src="media/image-20220520180300653.webp" alt="image-20220520180300653" style="zoom:50%;" />

#### 直接存储器访问 DMA 控制方式

DMA 控制器、数据传输单位扩大

| <img src="media/image-20220520200404061.webp" alt="image-20220520200404061" style="zoom: 67%;" /> | <img src="media/image-20220520200434052.webp" alt="image-20220520200434052" style="zoom: 33%;" /> |
| ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
...
<img src="media/image-20220520200503330.webp" alt="image-20220520200503330" style="zoom:60%;" />

### 设备管理目标、功能及结构

设备管理程序用于控制与管理实现信息输入、输出和存储的设备。

主要目标：

- 方便用户使用，实现对不同类型设备的统一管理、使用并协调各台外围设备的并发运作
- 提高系统I/O效率，缓解CPU和多种不同处理速度的外围设备之间的矛盾
...
- **用户空间I/O请求层** 
  提供对逻辑设备的控制。
  具体来说，针对用户接口，提供抽象的命令，如Open, Close, Read, Write；
  针对通信设备，提供通信体系结构如网络协议栈；
  针对文件存储设备，提供文件系统的逻辑结构控制。
- **设备独立性软件层** 
  逻辑设备与物理设备间过渡协调机构，实现用户命令到设备操作序列的转换，提供缓冲机制
- **设备调度和控制层** 
  实现硬件物理设备的设备驱动、设备控制、状态维护、中断处理及并发I/O访问调度

<img src="media/image-20220520231425271.webp" alt="image-20220520231425271" style="zoom:50%;" />

### 缓冲管理

#### 缓冲技术的引入

- 缓冲的功能：降低中断频率、放宽中断服务时间限制、提高IO与CPU并行性、缓解IO与CPU速度不匹配的矛盾
- 需要的硬件设备：设备控制器的数据缓冲寄存器，暂存输入输出数据

| 1 位缓冲                                                          | 8 位缓冲寄存器                                                       | 8 位缓冲寄存器和暂存                                                    |
| -------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------- |
| ![image-20220521134256315](media/image-20220521134256315.webp) | ![image-20220521134313116](media/image-20220521134313116.webp) | ![image-20220521134330122](media/image-20220521134330122.webp) |

#### 单缓冲

系统对每一整块数据的处理时间为 $max(C, T) + M$

<img src="media/image-20220521140857317.webp" alt="image-20220521140857317" style="zoom:50%;" />

<img src="media/image-20220521140344249.webp" alt="image-20220521140344249" style="zoom:50%;" />

#### 双缓冲

系统对每一整块数据处理时间为 $max(M+C , T)$

<img src="media/image-20220521141724582.webp" alt="image-20220521141724582" style="zoom:50%;" />
...
- 进程同步
  - Nexti 指针追赶上 Nextg 指针时，输入进程应阻塞，直至计算进程把某满缓冲区数据取走并调用 ReleaseBuf 过程将之唤醒
  - Nextg 指针追赶上 Nexti 指针时，计算进程应阻塞，直至输入进程装满某空缓冲区并调用 ReleaseBuf 过程将之唤醒

<img src="media/image-20220521141920352.webp" alt="image-20220521141920352" style="zoom: 33%;" />
...
<img src="media/image-20220521142927496.webp" alt="image-20220521142927496" style="zoom: 50%;" />

### 设备分配

#### 数据结构

**系统设备表 SDT**：记录系统全部设备的情况，包括设备类型、设备标识符、设备控制表指针、设备驱动程序入口

**设备控制表 DCT**：针对每台设备而设置和记录对应情况，包括设备类型、设备标识符、设备状态忙/闲、设备连接的设备控制器控制表的指针、重复执行次数或时间、设备请求队列队首及队尾指针

**设备控制器控制表 COCT**：针对每个设备控制器而设置和记录对应情况，包括设备控制器标识符、设备控制器状态忙/闲、设备控制器连接的通道控制表的指针、设备控制器请求队列队首/队尾指针

**通道控制表 CHCT**：包括通道标识符、通道状态忙/闲、通道连接的设备控制器控制表的指针、通道请求队列队首/队尾指针

### 设备处理

设备独立性指的是用户编写程序时所使用的逻辑设备名称与实际使用的物理设备无关，由操作系统建立物理设备名称和逻辑设备名称的关系，为用户提供统一的接口

#### 假脱机技术(SPOOLing)

<img src="media/image-20220521144216258.webp" alt="image-20220521144216258" style="zoom:50%;" />
...
- **固定头磁盘**
  在每条磁道上都有一个读写磁头，所有磁头装在一刚性磁臂上，通过这些磁臂可访问所有的磁道，并进行并行读写；
  能有效提高磁盘I/O速度，这种结构主要用于大容量磁盘
- **活动头（移动头）磁盘**
  每个盘面仅配有一个读写磁头，也被装入磁臂中，为了能访问该盘面上的所有磁道，该磁头必须能够移动和进行寻道；
  移动头磁盘结构简单，但只能进行串行读写故而I/O速度较慢，多用于中小型磁盘设备中

#### 磁盘数据的组织和格式

##### 低级格式化

- 功能：介质检查；磁盘介质测试；划分磁道和扇区；对每个扇区进行编号（C/H/S）
- 特点：
  - 只能在DOS环境或自写的汇编指令下进行
  - 低级格式化只能整盘进行
  - 现在硬盘出厂都是经过低格的，实际使用不要使用低格。

<img src="media/image-20220525202956095.webp" alt="image-20220525202956095" style="zoom:40%;" />
...
- **旋转延迟时间** $T_r= 1/(2r)$
  指定扇区旋转到磁头下面所经历的时间，其中 $r$ 为转速
  大白话：**硬盘转半圈的时间**
- **数据传输时间** $T_t = bytes/(r×bytesPerTrack)$
  把 $bytes$ B 数据从每个磁道 $bytesPerTrack$ B 的磁盘读出、或向磁盘写入数据所经历的时间 
  访问时间中，数据传输时间所占比例相当小，而寻道时间和旋转延迟时间基本上均与所读写的数据量无关，所以适当地集中数据传输（不要太零散）将有利于提高传输效率

#### 磁盘调度算法

1. **先来先服务调度算法 FCFS**：先调度先来的进程
2. **最短寻道时间优先调度算法 SSTF**：优先调度与磁头当前所在磁道距离最近的进程
3. **电梯调度（扫描）算法 SCAN**：向外移动时找外面最近的，向内移动时找内侧最近的
4. **循环电梯调度（扫描）算法 CSCAN**：向外移动，外侧没有了就移动到最内侧的请求
5. **双队列电梯调度（扫描）算法 FSCAN**：分当前和等待 2 个队列，当前队列先使用 SCAN

#### 磁盘高速缓冲

概念：利用内存中的存储空间来暂存可能会被经常访问的磁盘中盘块内容（不是CPU与内存之间的cache），进程读写磁盘时，首先查看磁盘高速缓冲是否有访问的数据（命中），没有时（不命中）才访问物理磁盘。
...
采用最近最久未使用置换算法 LRU，考虑访问频率、可预见性及数据一致性原则

周期性写回磁盘，防止经常被访问的盘块数据一直存放在高速缓冲不被写回磁盘而可能随故障发生丢失

##### 写穿透高速缓存 Write-Through Cache
...
- **提前读**
  用户（进程）对文件访问常采用顺序访问方式，即顺序地访问文件各盘块的数据，所以在读入当前块的同时，可将下一盘块数据提前读入缓冲区
- **延迟写**
  指缓冲区中的数据本应立即写回磁盘，但考虑到它们不久之后可能还会再被本进程或其它进程访问，因而并不立即将其写入磁盘，而是将它挂在空闲缓冲区队列的末尾
- **虚拟盘（RAM盘）**
  指利用内存空间仿真磁盘，常用于存放临时文件
  与磁盘高速缓冲区别：内容由用户/OS控制

#### 廉价磁盘冗余阵列 RAID

把多个（几台~几十台）廉价的磁盘采用一定的方式组合起来，利用一台磁盘阵列控制器来统一管理和控制，组合成一个大容量、高可靠、高性能的逻辑磁盘。

- **RAID1级**
  工作盘和镜像盘，可靠性高
  只有50%的空间利用率
- **RAID2级**
  海明校验，2^N^ 磁盘作为校验盘，其他是数据盘
- **RAID3级**
  一个盘做奇偶校验盘
  其他盘采用交叉存取
...
- 每一盘块数据分为N个子盘块数据，并把每个子盘块数据分别存储到各个不同磁盘中的相同位置；以后一个盘块数据向内存传送时采用并行传输方式。
- 性能高（比单个磁盘读写速度提高N倍）
- 无空间浪费，无冗余保护
...
- 包括类型，长度，物理位置等属性
- 操作系统通过抽象存储设备的属性提供统一的文件读写、创建、删除、共享和保护方法
- 操作系统为每个文件提供文件名称，从而允许用户按不同的名字访问对应的文件。

文件管理

- 提供方便用户使用的管理方式，并保证文件的安全性，提高资源的空间利用率；
- 作为用户程序和文件、目录、磁盘之间的抽象层，为上层的用户提供容易使用的命令接口和程序接口；

<img src="media/image-20220602141701657.webp" alt="image-20220602141701657" style="zoom: 50%;" />

### 文件的逻辑结构

逻辑结构、物理结构两者可以完全独立，目标是提高检索效率、方便修改、降低文件存储成本

物理结构：文件的存储结构，文件在外存上的组织方式。

逻辑结构：用户视角出发观察到的文件的组织形式，是用户可以直接处理的数据及其结构。
...
逻辑记录的排序：串结构与顺序结构(关键词排序)

**顺序文件读写操作**

1. 读写指针 `RWptr <=> 对应记录逻辑地址`
2. 定长记录 `RWptr += recordLength`
3. 定长记录 `RWptr = i*L`
4. 变长记录 `RWptr += currentRecordLength`
...
<img src="media/image-20220602143010241.webp" alt="image-20220602143010241" style="zoom:50%;" />

#### 索引顺序文件

对于拥有N条记录的主数据文件，若基于顺序查找法来检索具有指定关键字的记录，不同文件组织方式下的系统检索开销比较：

1. 顺序文件组织方式：`(N+1)/2` 条
2. 索引文件组织方式：`(N+1)/2` 条
3. 索引顺序文件组织方式：`sqrt(N)+1` 条（其中分组大小 `sqrt(N)` 条记录）

<img src="media/image-20220602143103313.webp" alt="image-20220602143103313" style="zoom:50%;" />
...
- 索引顺序文件组织方式
  ...
  - 分组大小1000条记录
- 两级索引顺序文件组织方式
  - 主文件分组大小100条记录[CubicRoot(N)]
  - 低级索引表分组大小100条记录[CubicRoot(N)]
  - 检索开销151.5条[1.5*CubicRoot(N)+1.5]
...
| 外存分配方式 | 示意图                                                                                              |
| ------ | ------------------------------------------------------------------------------------------------ |
| 连续分配   | <img src="media/image-20220602145527960.webp" alt="image-20220602145527960" style="zoom:50%;" /> |
| 隐式链接   | <img src="media/image-20220602150133509.webp" alt="image-20220602150133509" style="zoom:50%;" /> |
| 显式链接   | <img src="media/image-20220602150156825.webp" alt="image-20220602150156825" style="zoom:50%;" /> |
| 索引分配   | <img src="media/image-20220602150257798.webp" alt="image-20220602150257798" style="zoom:50%;" /> |

#### 连续分配

存在外部碎片问题（磁盘空间分割后形成的较小的无法存储文件的连续区）需要紧凑

优点：顺序访问容易且速度快、支持直接存取

缺点：要求有连续的存储空间，空间利用率低；必须事先知道文件的长度；

#### 链接分配

支持离散分配方式，通过盘块上的指针实现同一文件多个离散盘块的链接
...
1. 只适合顺序访问，对随机存取极其低效
2. 通过链接指针实现离散各盘块的链接
3. 只要其中任何一个指针出现问题，都会导致整条链的断开可靠性较差

改进方式：为了提高检索速度和减少指针所占用的存储空间，可将几个盘块组成一个簇，以簇为单位进行盘块分配，但会导致内部碎片增大

#### 索引分配

解决链接分配方式的缺陷

文件打开仅须把该文件所占用盘块编号调入内存即可，故可将每个文件所对应的盘块号集中地存放一个所谓的索引块中，形成一个索引表

缺点：对于小文件，索引块利用率极低

#### 多级索引

为解决单级索引的低效引入多级索引分配方式
...
空闲盘区的回收：考虑回收区域插入点前后盘块区是否相邻接，相邻则予合并
...
- **空闲盘区链**
  以盘区为基本元素链接
  每个盘区除含有指示下一个盘块区的指针外，还应标有指明本盘区大小的信息
  分配与回收
  为提高对空闲盘区的检索速度，可采用显式链接方式

#### 位示图法（Bitmap）

利用位示图（即二维数组 Map[m, n]）的一位（0/1）来表示磁盘中一个盘块的使用情况；

磁盘上所有盘块都与一个二进制位相对应；

<img src="media/image-20220619000124733.webp" alt="image-20220619000124733" style="zoom: 50%;" />

#### 成组链接法

<img src="media/image-20220619000155134.webp" alt="image-20220619000155134" style="zoom:50%;" />
...
- 存取控制信息（各类用户存取权限）
- 使用信息（建立/上次修改日期及时间）
- 当前使用信息（当前已打开该文件的进程数、是否被其它进程锁住、文件在内存是否已修改尚未拷贝到盘上）

> MS-DOS文件控制块内容例：
> 
> 1. 文件控制块长度为32字节
> 2. 文件名及文件扩展名共11个字符
> 3. 对于360KB的软盘，总共含有112个FCB，故占有4KB的存储空间
> 
> | 文件名 | 文件扩展名 | 文件属性 | 备用  | 文件建立时间 | 文件建立日期 | 文件所在第一盘块号 | 文件长度（盘块数） |
> | --- | ----- | ---- | --- | ------ | ------ | --------- | --------- |

#### 索引结点

引入索引结点的必要性：文件目录存放在磁盘上，且可能要占用大量盘块N ，检索开销很大（盘块调入次数 [N+1] /2 ）
可行性：只有文件名对目录检索有用

### 文件共享与保护

文件共享：指系统应允许多个用户（进程）共享同一份文件，从而在系统中只需保存该共享文件的一个副本即可
